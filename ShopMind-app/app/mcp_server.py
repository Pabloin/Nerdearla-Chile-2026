"""
mcp_server.py — ShopMind MCP Server
Exposes real-time MercadoLibre Chile scraping tools via MCP protocol.
...
mcp = FastMCP("shopmind-tools")

_SCRAPER_PATH = os.path.join(os.path.dirname(__file__), "agents", "scraper.js")
...
def _scrape(query: str, limit: int = 10) -> list:
    """Run Puppeteer scraper, persist results, return product list."""
    try:
...
@mcp.tool()
async def price_compare(product_name: str, max_price_usd: float) -> str:
...
@mcp.tool()
async def fetch_reviews(product_name: str, focus_audience: str = "general") -> str:
...
@mcp.tool()
async def user_memory(user_id: str, action: str, data: str = "") -> str:
    ...
    if action == "get":
        ...
        _user_prefs[user_id] = json.loads(data) if data else {}
        return json.dumps({"saved": True, "user_id": user_id}, ensure_ascii=False)


if __name__ == "__main__":
    import uvicorn
    print("ShopMind MCP Server starting on http://localhost:8001")
    print("Tools: web_search, price_compare, fetch_reviews, user_memory")
